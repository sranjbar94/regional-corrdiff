...
#SBATCH --mem=60G
#SBATCH --time=5:50:00
#SBATCH --output=slurm_g3_eval_16ens_%j.log
...
echo "============================================================"
echo "G3 Radiation — Evaluation (ssrd, strd)"
echo "16 ensemble members, 3k samples"
echo "============================================================"
echo "Node: $(hostname)"
echo "GPU:  $(nvidia-smi --query-gpu=name --format=csv,noheader)"
echo "Started: $(date)"
REG_CKPT=$(ls -t checkpoints_g3_regression/CorrDiffRegressionUNet.*.mdlus 2>/dev/null | head -1)
DIFF_CKPT=$(ls -t checkpoints_g3_diffusion/EDMPrecondSuperResolution.*.mdlus 2>/dev/null | head -1)
...
echo ">>> G3 CorrDiff evaluation: $TOTAL timestamps x 16 ensemble"
...
for (( c=0; c<N_CHUNKS; c++ )); do
    ...
    torchrun --standalone --nproc_per_node=1 generate.py \
        --config-name=config_generate_g3.yaml \
